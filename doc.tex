...
Secondly, for four samples, labeled A, B, C and D, determine either the material in question or the orientation of the crystal plane of said material.
...
$d(hkl)$ can be normalized and shown to be equal to $\frac{2\pi}{\abs{\bold{G}}} = \frac{a}{\sqrt{h^2+k^2+l^2}}$ \cite{Kittel}.
...
The scattering vector $\bold{q}$ equals the difference between the outgoing and incoming wavevectors,
...
This occurs since when $\bold{q}$ differs significantly from $\bold{G}$, $F$ will becomes negligibly small.
...
In short, the locus of scattering vectors in reciprocal space is a line segment along the Miller index of the crystal plane cut.
...
For the NaCl measurement, a time step of 1 s and incident angles from 2.5$\degree$ to 30$\degree$ were used.
...
The peaks were thereafter semi-automatically detected by a peak detector
heuristic commonly used for peak detection in mass
spectroscopy\cite{peakdetection}, then the spuriously detected peaks was
then manually excluded when used for calculations and such.
Directly one can observe that the first maximum of the plots - with a much broader peak - come from the bremsstrahlung and do not concern this experiment.
These peaks are therefore omitted from the tables of identified peaks.


\subsection{Structure factors}
The structure factor must be nonzero for a diffraction peak to appear.
The structure factors $S_{\bold{G}}$ of crystal structures are calculated here using methods given in \cite{Kittel}. Here $n$ is the number of atoms in the basic lattice, and atom $j$ is at position $\bold{r}_j$ with atomic form factor $f_j$. The atomic form factors are approximately equal to the atomic number.\\
\begin{equation}
...
However, as the reflected radiation intensity is proportional to the structure
factor, a small structure factor indicates smaller intensities.
...
This structure factor is $0$ whenever $S^{fcc}_{\bold{G}}=0$. It also disappears when $h+k+l\equiv_4 2$ as this makes $\left( 1 + e^{\pi i \frac{h+k+l}{2}} \right)=0$.\\
Si has lattice parameter $5.43$ Å.

\subsubsection{InP and GaAs}
The remaining two crystals in this lab both have a zinc blende structure with two atomic factors, $f_\alpha$ and $f_\beta$.
...
\plot{NaClpeaks}{Diffractogram with $\beta-2\beta$ scan of sample NaCl with
automatically extracted peaks\cite{peakdetection} (not all are considered diffraction peaks) and their corresponding values.}
\plot{Apeaks}{Diffractogram with $\beta-2\beta$ scan of sample A with automatically extracted peaks \cite{peakdetection}(not all are considered diffraction peaks) and their corresponding values.}
\plot{Bpeaks}{Diffractogram with $\beta-2\beta$ scan of sample B with automatically extracted peaks \cite{peakdetection}(not all are considered diffraction peaks) and their corresponding values.}
\plot{Cpeaks}{Diffractogram with $\beta-2\beta$ scan of sample C with automatically extracted peaks \cite{peakdetection}(not all are considered diffraction peaks) and their corresponding values.}
\plot{Dpeaks}{Diffractogram with $\beta-2\beta$ scan of sample D with automatically extracted peaks \cite{peakdetection}(not all are considered diffraction peaks) and their corresponding values.}
...
Given the X-ray wavelength, equation \eqref{eq:braggs} can be used to find the expected angles for different materials and crystal cuts. In subsequent calculations the mean $\hat{\lambda}$ is used for $\lambda$.
...
The structure factor of Si disappears when $h+k+l\equiv_4 2$, which happens for $m=2,6,10,\dots$.
...
These diamond/zinc blende crystals must be cut along planes with Miller indices $100$, $110$ or $111$, of which the $hkl$-triples then are multiples.
...
Theoretical GaAs peaks for which the structure factor is almost 0 are within
parenthesis.
